...
`cumulative_2006_2016.dta` can be read by Stata, or in R by the `haven` package 
...
* Limitations: Only available for even years. The third party candidate not specified for early years. The fourth candidate and onwards not shown for most years. 
...
```{r, size="small"}
filter(df, year == 2012, st == "CA", dist_up == 43) %>% select(year, case_id, st, dist_up, matches("intent_rep")) %>% sample_n(10)
```
