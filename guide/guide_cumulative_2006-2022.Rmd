...
thanks: "Department of Political Science, Yale University. Website: \\url{https://www.shirokuriwaki.com}. ORCID: \\url{https://orcid.org/0000-0002-5687-2647}. My thanks to Alexander Agadjanian, Jeremiah Cha, Steve Ansolabehere, Stephen DiMauro, Bernard Fraga, Nathan Kaplan, Silvia Kim, Mayya Komisarchik, Stephen Pettigrew, Boris Shor, Brian Schaffner, and Gerlad Wright for finding errors and providing suggestions. Thanks to Joe Williams at YouGov, and Jon Keane,  Mike Malecki, and Gordon Shotwell at Crunch for their help."
...
cc <- readRDS(find_rstudio_root_file("data/release/cumulative_2006-2022.rds"))
...
in_years <- function(var, tbl = cc) {
  ...
  if (identical(yrs_vec, 2007:2022)) return("All of 2007-2022")  
  if (identical(yrs_vec, 2008:2022)) return("All of 2008-2022")  
  if (identical(yrs_vec, setdiff(c(2006:2022), 2007))) return("All of 2006-2022 except 2007")  
...
> Kuriwaki, Shiro, 2023, "Cumulative CCES Common Content", \href{https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/II2DB6}{\url{doi:10.7910/DVN/II2DB6}}, Harvard Dataverse, V8. 

\medskip

This dataset combines `r english::english(2022 - 2006 + 1)` years (2006 -- 2022) of the Cooperative Congressional Election Study (CCES), renamed the Cooperative Congressional Election Study (CES) from 2020. The CCES/CES is an online survey conducted around November of each year, asking a range of questions on political behavior and public opinion. Its current principal investigators are Stephen Ansolabehere, Brian Schaffner, and Marissa Shih.
...
cc <- readRDS("cumulative_2006-2021.rds")
```
...
\noindent  A Stata dta version is provided as well. `cumulative_2006-2022.dta` can be read by Stata, or in R by the `haven` package 

```{r, eval = FALSE, echo = TRUE}
library(haven)
cc <- read_dta("cumulative_2006-2022.dta")
```
...
A version of the dataset is also included in Crunch, a database platform that makes it easy to view and analyze survey regardless of programming experience. 
...
* Although called `gender` for historically, this is a question asking people's sex. Until 2020, only two choices were allowed. From 2021, the question was discontinued in favor of the question now called `gender4`. I have created a variable `sex` to more accurately capture what the `gender` question has asked 2006--2020. 
* This `gender` maintains a binary variable for all years by making the following custom recodes for 2021 onwards: `Man` in `gender4` is recoded to `Male`, `Woman` in `gender4` is recoded to `Female`, and other categories are coded as missing.

### `r nam(gender4)`
`r descrip(gender4)`
```{r}
print_numeric(gender4)
```
...
* In years in which this question was fielded, this question adds to the `race` variable by asking those who did _not_ respond "Hispanic" in the `race` question. For a combined version of `race` and `hispanic`, see `race_h` (any-part Hispanic).
...
* In the above table, we separately count the occurence of each of the options. For example, the respondent in the above example will be counted twice in the table.
* This question is asked only to those who identify as any-part Hispanic in the `race` and `hispanic` questions, with a few exceptions in 2016 and 2018 for multi-racial candidates.
...
Routine edits add new rows, add new variables, and change the customization of existing variables. The version history is explained below and old versions can be downloaded from Dataverse. Dataverse assigns version numbers by incrementing a full number if any of the datasets change, and an incrementing decimal when the description changes.

### Version 8.0 

- Released `2023-05-01`
- Added `gender4` (a variable for gender identification introduced in 2021), add back `gender` for 2021-2022, and add `sex` for 2006-2020.
