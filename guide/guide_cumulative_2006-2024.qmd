...
Downloading datasets that are hundreds of megabytes large can take multiple minutes each time. The total time it takes to download is a combinaiton of the (1) download time, and (2) time for reading the downloaded file into R. As of 2025, R users can skip the step 1 after the first time by asking the dataverse package to save a cache of the data to the computer.
...
### Version 11.0, _planned_

- This version will include vote validation for 2024. It may also include new variables on voter knowledge and voter activity.  Historically, vote validation files are released about a year after the initial release. Please contact me for any questions.

### Version 10.0 (released `2024-05-21`)

- Adds 2024 Common Content (before vote validation), up to n = 701,955
- Fixes a bug in `voted_pres_party` where past election vote was being filled in for some respondents who did not take the post-election wave in that year. Add "Did not vote" (turnout) and undervoting in Presidential vote
